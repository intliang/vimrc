...
" 设置代码配色方案
if g:isGUI
    "colorscheme base16-default-dark               "Gvim配色方案
    colorscheme desert
else
...
"let FIND_PATTERN = '-iname "*.h" -o -iname "*.c" -o -iname "*.cpp" -o -iname "*.cc" -o -iname "*.hpp" -o -iname "*.m" -o -iname "*.mm" -o -iname "*.java" -o -iname "*.sh" -o -iname "*.py"'

function! GenerateSrcFile()
    let x = delete("cscope.files")
    let x = delete("cscope.out")
    let x = delete("ncscope.out")
    if(has('win32'))
        silent! execute "!dir /b /s *.c,*.cc,*.cpp,*.cxx,*.h,*.hpp,*.m,*.mm,*.java,*.py,*.sh,*.as > cscope.files"
        "silent! execute "!dir /b /s > cscope.files"
    else
        silent! execute '!find $PWD -iname "*.h" -o -iname "*.c" -o -iname "*.cpp" -o -iname "*.cc" -o -iname "*.hpp" -o -iname "*.m" -o -iname "*.mm" -o -iname "*.java" -o -iname "*.sh" -o -iname "*.py" -o -iname "*.go" > cscope.files'
    endif
endfunction

function! GenerateCsfile()
    call GenerateSrcFile()
    
    let bytes = getfsize("cscope.files")
    if (bytes > 0)
        execute "cs reset"
        let ret = delete("cscope.out")
...
if(executable("cscope") && has("cscope") )
    ...
    "nmap <F11> :!cscope -b -q -i cscope.files -f cscope.out<CR>:cs reset<CR>
endif
...
function! CleanTmpFiles()
	...
	execute "cs kill ncscope.out"
	execute "cs reset"
	let ret = delete("cscope.files")
	let ret = delete("cscope.out")
	let ret = delete("ncscope.out")
	
	"exe 'silent !del "cscope.out" '
	"exe 'silent !del "cscope.files" '
endfunction

autocmd VimLeave * :call CleanTmpFiles()
